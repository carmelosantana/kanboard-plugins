...
- **Double border on header project selector** — the broad `input[type="text"]`
  rule added a border to the inner `input.select-dropdown-input`, while
  Kanboard's native CSS already borders the `.select-dropdown-input-container`
  wrapper, producing a double box. The inner input is now reset to `border: none`
  with no focus box-shadow.
